...
class DeepGCN(torch.nn.Module):
    def __init__(self, opt):
        ...
        # 將張量移動到 CUDA（GPU）上，如果可用的話
        # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.Selfattention = SelfAttention(input_dim=1024)
        # self.Selfattention = self.Selfattention.to(device)
    ...
    def forward(self, inputs):
        ...
        # return self.prediction(x3).squeeze(-1).squeeze(-1)

        """ my design """
        fusion = fusion.squeeze()
        fusion = self.Selfattention(fusion).unsqueeze(-1)
        x1 = F.adaptive_max_pool2d(fusion, (1,1))
        x2 = F.adaptive_avg_pool2d(fusion, (1,1))
        x3 = torch.cat((x1, x2), dim=1) # x3 is (batch, 2048)
        return self.prediction(x3).squeeze(-1).squeeze(-1)
...
if __name__ == '__main__':
    ...
    feats = torch.rand((10, 3, 1024), dtype=torch.float).to(args.device)
    num_neighbors = 20
